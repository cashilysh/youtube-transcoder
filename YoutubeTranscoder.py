...
2-level black & white palette (1 bit/block):
  ...
  Palette (luma levels, decision boundary at luma 127.5):
    Symbol 0 -> Black (  0,   0,   0)   luma = 0
    Symbol 1 -> White (255, 255, 255)   luma = 255

  The single decision boundary gives ~127-128 luma units of noise margin.
  YouTube's Y-channel compression drifts uniform blocks by at most ~15 luma
  units, well inside that margin.
...
CPU encoder (default) -- libx265 via ffmpeg:
  Flags tuned for uniform B&W block data:
    all-intra (keyint=1), no B-frames (bframes=0),
  ...
  Size controlled entirely by CPU_BITRATE (default 80 Mbps, ABR mode).
  Speed/size trade-off controlled by CPU_PRESET (default 'superfast').
...
GPU encoder (--nvenc) -- hevc_nvenc via ffmpeg:
  ...
  Quality controlled by NVENC_QP (default 4; safe range 0-14; above 14 risks
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  NVENC_QP     hevc_nvenc constant QP  (default 4; safe range 0-14)
...
# Try to enable Windows Virtual Terminal Processing once at import time.
# Caching the result avoids repeated kernel32 calls on every ANSI escape.
_ANSI_OK = True   # non-Windows always supports ANSI
if _IS_WINDOWS:
    try:
        import ctypes
        _kernel32 = ctypes.windll.kernel32
        _stdout_handle = _kernel32.GetStdHandle(-11)   # STD_OUTPUT_HANDLE
        _vtp_mode = ctypes.c_ulong()
        _kernel32.GetConsoleMode(_stdout_handle, ctypes.byref(_vtp_mode))
        _kernel32.SetConsoleMode(_stdout_handle, _vtp_mode.value | 0x0004)
        _ANSI_OK = True
    except Exception:
        _ANSI_OK = False   # fallback: progress lines print sequentially


def _ansi_cursor_up(n):
    """Move the terminal cursor up n lines (ANSI ESC[nA).
    No-op on Windows when Virtual Terminal Processing is unavailable."""
    if _ANSI_OK:
...
def encode(input_path, output_path, fps=30, k=12, m=15,
           preset='superfast', use_nvenc=False, qp=4,
           nvenc_preset='p4', bitrate=80_000_000):
    ...
    if use_nvenc:
        ...
        print(f"Encoder     : x265 (CPU)  |  preset: {preset}  |  ABR: {bitrate//1_000_000} Mbps")
    ...
    if use_nvenc:
        ...
        #   -qp N                0-14 is safe: palette spacing is 255 luma
        #                        units; constqp+HEVC drifts uniform blocks by
        #                        ≲10 units at QP 14, well inside ±127 margin.
        #                        Default is 4; values above 14 risk corruption.
        ...
        # clients and re-encodes VP9 as fallback.  At QP 4 the output
        # bitrate for 8K all-intra HEVC is well above YouTube's 8K VP9
        # target (~50 Mbps), so the VP9 re-encode has enough signal to
        # preserve the ±127-unit luma margins.
        vf = (f'format=yuv420p,'
        ...
        # the 80 Mbps ABR target this closely matches NVENC QP 4 output size.
        ...
        # ABR mode: -b:v sets the target bitrate; -maxrate/-bufsize enforce a
        # hard ceiling so the muxer stream never spikes above it.  A 1-second
        # VBV buffer is sufficient for all-intra content where every frame is
        # independent.  This is the sole rate-control path (no CRF fallback).
        cpu_cmd += [
            '-b:v', str(bitrate),
            '-maxrate', str(bitrate),
    ...
    data_frame_counter = 0   # counts data frames only (excludes the manifest)
    ...
    with _Cleanup(cleanup):
        ...
        for seg_idx in range(total_segments):
            ...
            for share_idx, share in enumerate(shares):
                ...
                if data_frame_counter % 10 == 0 or data_frame_counter == total_frames:
                    pct = data_frame_counter / total_frames * 100
                    cur_size = os.path.getsize(output_path) if os.path.exists(output_path) else 0
                    exp_size = (cur_size * total_frames / data_frame_counter) if cur_size else 0
                    elapsed  = time.time() - start_time
                    eta      = elapsed * (total_frames - data_frame_counter) / data_frame_counter
                    exp_str  = f"~{exp_size/1024**2:.1f} MB" if exp_size else "?"
                    print(f"  Frame {data_frame_counter:6d}/{total_frames}  ({pct:5.1f}%)  "
...
def classify_luma(grey_blocks):
    ...
    """
    # PALETTE_LUMA = [0, 255]; true midpoint is 127.5.
    # Using >= 128 gives symmetric margins: 128 units from Black (0->128),
    # 127 units from White (255->127).  Using >= 127 would mis-classify
    # luma=127 as White even though it is one unit closer to Black.
    syms = np.zeros(len(grey_blocks), dtype=np.uint8)
    syms[grey_blocks >= 128] = 1
    return syms


def symbols_to_bits(syms):
    """syms: (N,) uint8 -> bits: (N*BITS_PER_BLOCK,) uint8"""
...
def stream_raw_frames(video_path):
    ...
    decode_log = open(_tmp('ffmpeg_decode.log'), 'w', errors='replace')
    proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=decode_log)
...
def stream_raw_frames_from_url(url):
    ...
    stream_log = open(_tmp('ffmpeg_stream_decode.log'), 'w', errors='replace')
...
def decode_frame(args):
    ...
    hdr_bits = symbols_to_bits(hdr_syms[:_HEADER_BLOCKS])        # 184 bits (184 blocks x 1 bit)
...
def roundtrip(input_path, video_path, decoded_path, fps, k, m,
              workers, preset, use_nvenc, qp, nvenc_preset, verbose,
              bitrate=80_000_000):
    print("=" * 72)
    ...
    encode(input_path, video_path, fps=fps, k=k, m=m,
           preset=preset, use_nvenc=use_nvenc, qp=qp,
...
# byte-for-byte.  Constraints: ZFEC_K < ZFEC_M <= 255.
#
# K=12, M=15 (default): tolerates 3 bad frames per segment (~20% loss budget).
#   Suitable when the encoded video is stored/transferred with high integrity
#   (local file, direct download).  Smaller storage overhead than K=10.
#
# K=10, M=15 (alternative): tolerates 5 bad frames per segment (~33% loss).
#   Use if you see decode failures on YouTube uploads -- YouTube applies two
#   lossy re-encode passes (HEVC → VP9) that can corrupt 3-5 frames per
#   segment in a single cluster, which exhausts the K=12 budget.
...
#               x265 runs in ABR mode capped to this target.  This is the
...
CPU_BITRATE = 80_000_000   # bits/sec
...
def build_parser():
    p = argparse.ArgumentParser(
        ...
        description=(
            ...
            'Encoder settings (ZFEC_K/M, FPS, bitrate, QP, presets) are '
...
def main():
    ...
    if args.encode:
        ...
        encode(args.input, output, fps=ENCODE_FPS, k=ZFEC_K, m=ZFEC_M,
               preset=CPU_PRESET, use_nvenc=args.nvenc,
        ...
        roundtrip(args.input, video_path, decoded_path,
                  ...
                  workers=None, preset=CPU_PRESET,
